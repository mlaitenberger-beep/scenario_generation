...
class InputData:
    def __init__(self, data_hist, data_stress):
        ...
        self.data_stress = pd.read_csv(data_stress, header=0, dtype=float).to_numpy() if data_stress is not None else None
        self.data_hist_rel = None
        self.data_stress_rel = None
    ...
    def relative_change(self, eps=1e-12):
        ...
        # Only process stress data if it exists
        if self.data_stress is not None:
            # Stress: first step connects to last historical value
            last_hist = self.data_hist[-1]  # Shape: (features,)
            stress_prev = self.data_stress[:-1]
            stress_first_prev = last_hist  # Reference for t=0 in stress

            # Relative change for stress:
            # - first stress element relative to last_hist
            first_rel = (self.data_stress[0] - stress_first_prev) / (stress_first_prev + eps)

            # - remaining stress elements relative to previous stress value
            rest_rel = (self.data_stress[1:] - stress_prev) / (stress_prev + eps)

            # Combine -> length = len(stress)
    ...
    def normalize(self):
        ...
        # Only normalize stress data if it exists
